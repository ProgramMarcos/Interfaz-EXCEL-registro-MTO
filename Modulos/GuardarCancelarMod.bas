Attribute VB_Name = "GuardarCancelarMod"
'------------------------------------------------------------------
'------------------------------------------------------------------
'---    MARCOS LÓPEZ LÓPEZ
'---    2024/2025
'---    MODULO GuardarCancelarMod
'---    Contiene las subrutinas asignadas a los botones "Cancelar" y
'---    "Guardar" cuando se MODIFICA una hoja de reporte
'------------------------------------------------------------------
'------------------------------------------------------------------

Sub GuardarResultados()
    ...
    ' Referencia al libro actual (el que se abrió desde la carpeta "Results")
    Set libroActual = ActiveWorkbook
    
    ' Referencia al libro de origen
    Set libroOrigen = Workbooks(ThisWorkbook.name)
    ...
    ' Si no se pudo abrir, crea un nuevo libro y lo guarda
    ...
    Set ws = libroActual.ActiveSheet
    
    ' Referencia a la hoja "referenciasHoy" en el libro de origen
...
'    End If
    ...
    wsResultados.columns("A:O").AutoFit
    
    Dim color As Integer
    For color = 1 To 21 ' Desde la columna 1 (A) hasta la columna 21 (O)
...
'    ' Guardar la hoja en un nuevo libro en la carpeta con la fecha
'    ws.Copy
'    Set nuevoLibro = ActiveWorkbook
'    rutaArchivo = carpetaFecha & "\" & ws.Name & ".xlsx"
...
'    nuevoLibro.Close False
    ...
    ' Ocultar la hoja "Resultados" en el libro de origen
    'wsResultados.Visible = xlSheetVeryHidden
    
    ' Retornar al libro de origen
    libroOrigen.Activate
End Sub



Sub CancelarModificaciones()
    Dim libroActual As Workbook
    Dim libroOrigen As Workbook
    ...
    ' Llama al procedimiento cancelarCierre en el módulo control
    control.cancelarCierre
    ' Cerrar el libro actual sin guardar cambios
    libroActual.Close SaveChanges:=False
    
    ' Activar la hoja "TECNICO" en el libro de origen
    libroOrigen.Sheets("TECNICO").Activate
    ' borrar el bloqueo al terminar
    Kill blockMod
End Sub
